...
# Browsing and dnd
class ModelEditorItem_Project extends ModelEditorItem
    constructor: ( params ) ->
        super params
...
#                 @refresh()
    ...
    @new_project_button: ( this_instance ) ->
        ...
        new_dom_element
            ...
            onclick: ( evt ) ->
                date = new Date()
                name = prompt "Project name", date.toLocaleString()
                td = new TreeAppData
                td.new_session()                        
                ...
                fs.load_or_make_dir FileSystem._home_dir + "/__projects__", ( current_dir, err ) ->
                    current_dir.add_file name, td, model_type: "Session", icon: "session"
...
#         console.log "---"
    ...
    share_popup: ( evt, elem, name_folder ) ->
        ...
        fs.load "__users__/" + login, ( home, err ) ->
            if err
                return alert "Please enter a valid login (email adress --)"
            fs.load_or_make_dir "__users__/" + login + "/__projects__", ( inbox, err ) ->
                if err
                    return alert "??"
                nn = name_folder
                while inbox.has nn
