        ...
        if files.length > 0
            if FileSystem?
                ...
                for file in files
                    ...
                    if @ends_with( file.name, ".tiff" ) or @ends_with( file.name, ".tif" )
                        @model.add_tiff_file file.name, new Path, new Path file
                    else
...
#                             console.log @drag_source.get(), @selected_file
    ...
    share_popup: ( evt, elem, name_folder ) ->
        ...
        fs.load "__users__/" + login, ( home, err ) ->
            if err
                return alert "Please enter a valid login (email adress --)"
            fs.load_or_make_dir "__users__/" + login + "/inbox", ( inbox, err ) ->
                if err
                    return alert "??"
                nn = name_folder
                while inbox.has nn
