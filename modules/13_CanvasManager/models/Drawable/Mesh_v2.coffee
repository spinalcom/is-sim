...
class Mesh_v2 extends Drawable
    constructor: ( params = {} ) ->
        super()
    ...
    add_point: ( pos = [ 0, 0, 0 ] ) ->
        ind = @points.size(1) + 1
        @points.resize [ 3, ind ]
        @points.set_val [ 0, ind ], pos[0]
        @points.set_val [ 1, ind ], pos[1]
        @points.set_val [ 2, ind ], pos[2]       

    set_point: ( indice, pos = [ 0, 0, 0 ] ) ->
        @points.set_val [ 0, indice ], pos[0]
        @points.set_val [ 1, indice ], pos[1]
        @points.set_val [ 2, indice ], pos[2]  

    get_point: ( indice ) ->
        [ @points.get([ 0, indice ]), @points.get([ 1, indice ]), @points.get([ 2, indice ]) ]
    ...
    draw: ( info ) ->
        ...
        if info.ctx_type == 'gl'
            ...
            proj = for i in [ 0 ... @points.size(1) ]
                info.re_2_sc.proj [ @points.get [ 0, i ], @points.get [ 1, i ], @points.get [ 2, i ] ]
